...
Image::Image(unsigned width, unsigned height) : d_pixels(width * height), d_width(width), d_height(height) {}

Image::Image(string const &filename) { read_png(filename); }

// normal accessors
void Image::put_pixel(unsigned x, unsigned y, Color const &c) { (*this)(x, y) = c; }
Color Image::get_pixel(unsigned x, unsigned y) const { return (*this)(x, y); }

// Handier accessors
// Usage: color = img(x,y);
//        img(x,y) = color;
Color const &Image::operator()(unsigned x, unsigned y) const { return d_pixels.at(index(x, y)); }
Color &Image::operator()(unsigned x, unsigned y) { return d_pixels.at(index(x, y)); }

unsigned Image::width() const { return d_width; }

unsigned Image::height() const { return d_height; }

unsigned Image::size() const { return d_width * d_height; }
